...
public class Question3 : MonoBehaviour {		//Question3 script is attached to the Question3 GameObject in the Hierarchy, it is almost identical to Question1 with a few exceptions
											//Refer to the Question1 script for a more detailed explanation of this script

	public GUIText textHints;
	public GameObject Questions;		//the GameObject assigned to this script is the Question3 GameObject in the Prefabs folder
	...
	void OnTriggerEnter(Collider col){
		if (col.gameObject.tag == "Player" && Inventory.Answer == 2) {		//The conditions that need to be met are the Player Colliding with Question3 GameObject and when Answer equals two
			Selecting.canSelect = true;
			textHints.SendMessage ("ShowHint", "Question 3");
		...
		} else if (col.gameObject.tag == "Player" && Inventory.Answer == 1) {		//When Answer equals ones and the Player collides, then the message will appear on screen
	...
	void OnTriggerExit (Collider col){
		if (col.gameObject.tag == "Player") {
			...
			GameObject Question3 = GameObject.FindGameObjectWithTag ("Question3");
			Destroy (Question3);
