...
public class Question9 : MonoBehaviour {			//Question9 script is attached to the Question9 GameObject in the Hierarchy, it is almost identical to Question1 with a few exceptions
													//Refer to the Question1 script for a more detailed explanation of this script

	public GUIText textHints;
	public GameObject Questions;				//the GameObject assigned to this script is the Question9 GameObject in the Prefabs folder
	...
	void OnTriggerEnter(Collider col){
		if (col.gameObject.tag == "Player" && Inventory.Answer == 8) {	//The conditions that need to be met are the Player Colliding with Question9 GameObject and when Answer equals eight
			Selecting.canSelect = true;
			textHints.SendMessage ("ShowHint", "Question 9");
		...
		} else if (col.gameObject.tag == "Player" && Inventory.Answer == 7) {	//When Answer equals seven and the Player collides, then the message will appear on screen
			textHints.SendMessage ("ShowHint", "Question 9 \n Answer Question 8 first");
		}
	}
	...
	void OnTriggerExit (Collider col){
		if (col.gameObject.tag == "Player") {
			...
			GameObject Question9 = GameObject.FindGameObjectWithTag ("Question9");
			Destroy (Question9);
	...
	IEnumerator AnswerBox (){		//Answer4 is tagged with Right with the rest being tagged as false.
		if (answerbool == false) {
