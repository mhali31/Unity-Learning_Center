...
public class Question10 : MonoBehaviour {		//Question10 script is attached to the Question10 GameObject in the Hierarchy, it is almost identical to Question1 with a few exceptions
												//Refer to the Question1 script for a more detailed explanation of this script

	public GUIText textHints;
	public GameObject Questions;				//the GameObject assigned to this script is the Question10 GameObject in the Prefabs folder
	...
	void OnTriggerEnter(Collider col){
		if (col.gameObject.tag == "Player" && Inventory.Answer == 9) {	//The conditions that need to be met are the Player Colliding with Question10 GameObject and when Answer equals nine
			Selecting.canSelect = true;
			textHints.SendMessage ("ShowHint", "Question 10");
		...
		} else if (col.gameObject.tag == "Player" && Inventory.Answer == 8) {	//When Answer equals eight and the Player collides, then the message will appear on screen
	...
	void OnTriggerExit (Collider col){
		if (col.gameObject.tag == "Player") {
			...
			GameObject Question10 = GameObject.FindGameObjectWithTag ("Question10");
			Destroy (Question10);
