...
public class Question14 : MonoBehaviour {			//Question14 script is attached to the Question14 GameObject in the Hierarchy, it is almost identical to Question1 with a few exceptions
													//Refer to the Question1 script for a more detailed explanation of this script

	public GUIText textHints;
	public GameObject Questions;			//the GameObject assigned to this script is the Question14 GameObject in the Prefabs folder
	...
	void OnTriggerEnter(Collider col){
		if (col.gameObject.tag == "Player" && Inventory.Answer == 13) {	//The conditions that need to be met are the Player Colliding with Question14 GameObject and when Answer equals thirteen
			Selecting.canSelect = true;
			textHints.SendMessage ("ShowHint", "Question 14");
		...
		} else if (col.gameObject.tag == "Player" && Inventory.Answer == 12) {	//When Answer equals twelve and the Player collides, then the message will appear on screen
			textHints.SendMessage ("ShowHint", "Question 14 \n Answer Question 13 first");
		}
	}
	...
	void OnTriggerExit (Collider col){
		if (col.gameObject.tag == "Player") {
			...
			GameObject Question14 = GameObject.FindGameObjectWithTag ("Question14");
			Destroy (Question14);
