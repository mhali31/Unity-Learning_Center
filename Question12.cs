...
public class Question12 : MonoBehaviour {			//Question12 script is attached to the Question12 GameObject in the Hierarchy, it is almost identical to Question1 with a few exceptions
													//Refer to the Question1 script for a more detailed explanation of this script

	public GUIText textHints;
	public GameObject Questions;			//the GameObject assigned to this script is the Question12 GameObject in the Prefabs folder
	...
	void OnTriggerEnter(Collider col){
		if (col.gameObject.tag == "Player" && Inventory.Answer == 11) {	//The conditions that need to be met are the Player Colliding with Question12 GameObject and when Answer equals eleven
			Selecting.canSelect = true;
			textHints.SendMessage ("ShowHint", "Question 12");
		...
		} else if (col.gameObject.tag == "Player" && Inventory.Answer == 10) {	//When Answer equals ten and the Player collides, then the message will appear on screen
			textHints.SendMessage ("ShowHint", "Question 12 \n Answer Question 11 first");
		}
	}
	...
	void OnTriggerExit (Collider col){
		if (col.gameObject.tag == "Player") {
			...
			GameObject Question12 = GameObject.FindGameObjectWithTag ("Question12");
			Destroy (Question12);
