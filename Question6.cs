...
public class Question6 : MonoBehaviour {		//Question6 script is attached to the Question6 GameObject in the Hierarchy, it is almost identical to Question1 with a few exceptions
												//Refer to the Question1 script for a more detailed explanation of this script

	public GUIText textHints;
	public GameObject Questions;		//the GameObject assigned to this script is the Question6 GameObject in the Prefabs folder
	...
	void OnTriggerEnter(Collider col){
		if (col.gameObject.tag == "Player" && Inventory.Answer == 5) {	//The conditions that need to be met are the Player Colliding with Question6 GameObject and when Answer equals five
			Selecting.canSelect = true;
			textHints.SendMessage ("ShowHint", "Question 6");
		...
		} else if (col.gameObject.tag == "Player" && Inventory.Answer == 4) {	//When Answer equals four and the Player collides, then the message will appear on screen
			textHints.SendMessage ("ShowHint", "Question 6 \n Answer Question 5 first");
		}
	}
	...
	void OnTriggerExit (Collider col){
		if (col.gameObject.tag == "Player") {
			...
			GameObject Question6 = GameObject.FindGameObjectWithTag ("Question6");
			Destroy (Question6);
