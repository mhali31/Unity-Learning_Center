...
public class Question8 : MonoBehaviour {		//Question8 script is attached to the Question8 GameObject in the Hierarchy, it is almost identical to Question1 with a few exceptions
												//Refer to the Question1 script for a more detailed explanation of this script

	public GUIText textHints;
	public GameObject Questions;		//the GameObject assigned to this script is the Question8 GameObject in the Prefabs folder
	...
	void OnTriggerEnter(Collider col){		
		if (col.gameObject.tag == "Player" && Inventory.Answer == 7) {	//The conditions that need to be met are the Player Colliding with Question8 GameObject and when Answer equals seven
			Selecting.canSelect = true;
			textHints.SendMessage ("ShowHint", "Question 8");
		...
		} else if (col.gameObject.tag == "Player" && Inventory.Answer == 6) {	//When Answer equals six and the Player collides, then the message will appear on screen
			textHints.SendMessage ("ShowHint", "Question 8 \n Answer Question 7 first");
		}
	}
	...
	void OnTriggerExit (Collider col){
		if (col.gameObject.tag == "Player") {
			...
			GameObject Question8 = GameObject.FindGameObjectWithTag ("Question8");
			Destroy (Question8);
